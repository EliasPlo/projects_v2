...
class MusicPlayer:
    ...
    def create_ui(self):
        # Otsikko
        self.label = tk.Label(self.root, text="Ei kappaletta valittuna", wraplength=400)
        self.label.pack(pady=10)

        # Soittolista
        self.playlist_label = tk.Label(self.root, text="Soittolistat")
        self.playlist_label.pack(pady=5)

        self.playlist_box = tk.Listbox(self.root, height=6)
        self.playlist_box.pack(pady=5)
        self.playlist_box.bind('<<ListboxSelect>>', self.load_playlist)

        self.add_playlist_button = tk.Button(self.root, text="Lisää Soittolista", command=self.add_playlist)
        self.add_playlist_button.pack(pady=5)

        # Kappalelista
        self.song_label = tk.Label(self.root, text="Kappaleet")
        self.song_label.pack(pady=5)

        self.song_box = tk.Listbox(self.root, height=10)
    ...
    def select_song(self, event):
        ...
        if selection:
            ...
            self.label.config(text=f"Valittu tiedosto: {song_name}")
